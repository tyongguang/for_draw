...
with tf.name_scope("main") as main_scope:
    ...
    with tf.name_scope("layer1") as layer1_scope:
        ...
        bias1 = tf.Variable(np.ones((16))* 0.1, dtype=tf.float32, name="cnn_bias1")
        conv2d_output1 = tf.nn.bias_add(tf.nn.conv2d(inputs, filter1, [1, 1, 1, 1], "VALID") , bias1)
        conv2d_output1 = tf.contrib.layers.batch_norm(conv2d_output1, decay=0.9, scope=layer1_scope, is_training = is_training)
    ...
    with tf.name_scope("layer2") as layer2_scope:
        ...
        bias2 = tf.Variable(np.ones((32))* 0.1 , dtype=tf.float32, name="cnn_bias2")
        conv2d_output2 = tf.nn.bias_add(tf.nn.conv2d(relu_output1, filter2, [1, 1, 1, 1], "VALID") , bias2)
        conv2d_output2 = tf.contrib.layers.batch_norm(conv2d_output2, decay=0.9, scope=layer2_scope, is_training = is_training)
    ...
    with tf.name_scope("fc1"):
        ...
        fc_b1 = tf.Variable(np.ones((512))* 0.1 , dtype=tf.float32, name="fc_b1")
        fc_ouptut1 = tf.nn.relu(tf.matmul(flatten_output, fc_w1) + fc_b1)

    with tf.name_scope("fc2"):
        fc_w2 = tf.Variable(tf.truncated_normal((512, 10)), name="w2")
        fc_b2 = tf.Variable(np.ones((10))* 0.1 , dtype=tf.float32, name="fc_b2")
...
with tf.Session() as sess:
    ...
    for i in range(TRAIN_STEPS):
        ...
        _, np_loss = sess.run([train_step, loss], feed_dict={inputs: train_images, labels: train_labels, is_training:True})

        if i % (TRAIN_STEPS/10) == 0:
            loss_value, step, acc_value, grad_list = sess.run([loss, global_step, acc, training_grads], 
                feed_dict={inputs: train_images, labels: train_labels, is_training:True})
...
with tf.Session() as sess:
    ...
    vars = sess.run(util.get_vars(grad_and_var), feed_dict={is_training:False})

    train_images, train_labels = mnist.train.next_batch(BATCH_SIZE)
    train_images =  train_images.reshape(-1, 28, 28, 1)
    grads = sess.run(util.get_grads(grad_and_var), feed_dict={inputs: train_images, labels: train_labels, is_training:True})
...
with tf.Session() as sess:
    ...
    target_def = util.save_as_pb("cnn_bn.pb", sess, ["x-input"], ["main/softmax/Softmax"])
